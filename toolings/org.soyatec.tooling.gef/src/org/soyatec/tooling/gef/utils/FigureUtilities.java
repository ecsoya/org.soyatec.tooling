 ...
 * geometry calculations that may be common across different parts of a
 * hierarchy.
 * 
 * @author sshaw
 */
public class FigureUtilities extends org.eclipse.draw2d.FigureUtilities {
	...
	/**
	 * Method colorToInteger. converts from a Color to an Integer representation
	 * 
	 * @param c
	 * @return Integer
	 */
	public static Integer colorToInteger(final Color c) {
		return new Integer((c.getBlue() << 16) | (c.getGreen() << 8)
				| c.getRed());
	}

	/**
	 * Method integerToColor. converts from an Integer to a Color representation
	 * 
	 ...
	 * @return Color
	 */
	public static Color integerToColor(final Integer i) {
		if (i == null) {
			return null;
		}
		final int color = i.intValue();
		return new Color(null, (color & 0x000000FF), (color & 0x0000FF00) >> 8,
				(color & 0x00FF0000) >> 16);
	}
	...
	/**
	 * Return the location within the supplied constraint the supplied rectangle
	 * requires to be positioned according to the alignment parameter
	 * <i>pos</i>.
	 * 
	 * <pre>
	 * NW-----N------NE
	 ...
	 * 
	 * @param pos
	 *            a geographic PositionConstant (N,E,W,S,NW,NW,NS,EW)
	 * @param topos
	 *            the rectangle to be (re)positioned.
	 * @param constraint
	 *            the containing bounds.
	 * @return a new location.
	 */
	public static Point getLocation(final int pos, final Rectangle topos,
			final Rectangle constraint) {
		final Rectangle b = constraint.getCopy();
		final Point svb = new Point(b.x, b.y);
	...
	/**
	 * Return the location within the supplied constraint of an object of
	 * dimension <i>dim</i> according to the alignment parameter.
	 * 
	 ...
	 * @return a new location.
	 */
	public static Point getLocation(final int pos, final Dimension dim,
			final Rectangle constraint) {
		return getLocation(pos, new Rectangle(0, 0, dim.width, dim.height),
				constraint);
	}
	 ...
	 * @param lineStyle
	 *            Line style to be used for painting the grid lines
	 * @param dashes
	 *            Dash pattern to be used for the grid line (ignored if
	 *            lineStyle != LINE_CUSTOM)
	 * 
	 */
	public static void paintGridWithStyle(final Graphics g, final IFigure f,
			final org.eclipse.draw2d.geometry.Point origin,
			final int distanceX, final int distanceY, final int lineStyle,
			final int[] dashes) {
		...
		if (distanceX > 0) {
			...
			for (int i = origin.x; i < clip.x + clip.width; i += distanceX) {
				...
				if ((dashes != null) && (lineStyle == SWT.LINE_CUSTOM)) {
	...
	/**
	 * Calculates the anchorable figure bounds. There could be cases that a
	 * figure implements both <code>IOvalAnchorableFigure</code> and
	 * <code>IPolygonAnchorableFigure</code> interfaces. The latter interface is
	 * more popular because any figure can be approximated with line segments
	 * including ellipse. Therefore, we need to check first if it's an ellipse
	 * like figure and then if it's some kind of a polygon.
	 * 
	 * @param figure
	 *            the figure
	 * @return the bounding <code>Rectangle</code> of the anchorable part of the
	 *         figure
	 */
	public static Rectangle getAnchorableFigureBounds(final IFigure figure) {
		if (figure instanceof IOvalAnchorableFigure) {
			...
			return ((IPolygonAnchorableFigure) figure).getPolygonPoints()
					.getBounds();
