...
public class LineSeg implements Cloneable, java.io.Serializable, Translatable {
	...
	static public class KeyPoint {
		...
		private KeyPoint(final String name) {
			this.name = name;
		}
	...
	static public class Sign {
		...
		private Sign(final String name) {
			this.name = name;
		}
	 ...
	 */
	public LineSeg(final Point ptStart, final Point ptEnd) {
	...
	public LineSeg(final KeyPoint start, final int fromX, final int fromY,
		...
		final double slope_squared = slope * slope;
	 ...
	 */
	public boolean equals(final Object seg) {
		if (!(seg instanceof LineSeg)) {
			return false;
		}

		final LineSeg ls = (LineSeg) seg;
	 ...
	 */
	public void setTerminus(final Point terminus) {
	...
	public Point intersect(final LineSeg line, final int nTolerance) {
		final PointList intersections = getLinesIntersections(line);
		if (intersections.size() > 1) {
			intersections.addPoint(getOrigin().getCopy());
			intersections.addPoint(getTerminus().getCopy());
		}
		for (int i = 0; i < intersections.size(); i++) {
			final Point result = intersections.getPoint(i).getCopy();
	...
	public final boolean containsPoint(final Point aPoint, final int tolerance) {
		 ...
		 */
		final double lengthOfSegment = origin.getDistance(terminus);
		final double lengthFromOriginToPoint = origin.getDistance(aPoint);
		final double lengthFromTerminusToPoint = terminus.getDistance(aPoint);
	 ...
	 */
	public final float distanceAlong(final Point coord) {
		final int xCoord = coord.x;
		final int yCoord = coord.y;
		...
		final long dirx = (long) terminus.x - (long) origin.x;
		final long diry = (long) terminus.y - (long) origin.y;

		/* get q - p */
		final long qpx = (long) xCoord - (long) origin.x;
		final long qpy = (long) yCoord - (long) origin.y;

		/* dot product of (q-p) and d */
		final long dotprod = qpx * dirx + qpy * diry;
	...
	public final long distanceToPoint(final int xCoord, final int yCoord) {
		...
		final double proj = projection(xCoord, yCoord);

		if (proj > 0 && proj < 1) {
			final Point pt = perpIntersect(xCoord, yCoord);
			return Math.round(pt.getDistance(new Point(xCoord, yCoord)));
		}

		final long d1 = Math.round(getOrigin().getDistance(
				new Point(xCoord, yCoord)));
		final long d2 = Math.round(getTerminus().getDistance(
				new Point(xCoord, yCoord)));
	...
	public final Point perpIntersect(final int startX, final int startY) {
		...
		//
		final Point ptResult = new Point();
		final float m = slope();
	...
	public final double projection(final int xCoord, final int yCoord) {
		...
		/* get the direction vector */
		final long dirx = (long) getTerminus().x - (long) getOrigin().x;
		final long diry = (long) getTerminus().y - (long) getOrigin().y;

		/* get q - p */
		final long qpx = (long) xCoord - (long) getOrigin().x;
		final long qpy = (long) yCoord - (long) getOrigin().y;
	 ...
	 *         given point.
	 */
	public final Sign positionRelativeTo(final Point rel) {
		final Ray ptRelRay = new Ray(getOrigin(), rel);

		final TrigValues val = getTrigValues(ptRelRay);
		if (val != null) {
			final double dNewAngle = Math.atan2(-val.sinTheta, -val.cosTheta);
			if (dNewAngle > 0) {
	...
	public final Point locatePoint(final double pctDist, final long theHeight,
		...
		int xdir;
		final int dist = (int) (pctDist * length());
		...
		final LineSeg linesegAB = new LineSeg(KeyPoint.ORIGIN, pt.x, pt.y,
	...
	public final boolean pointOn(final long theDistance,
			final KeyPoint fromKeyPoint, final Point ptResult) {
		float m, dx_float;
	...
	public TrigValues getTrigValues(final Ray ptToVector) {
		final double dFromLength = length();
		final double dToLength = ptToVector.length();

		final Ray ptFromVector = new Ray(getOrigin(), getTerminus());
		...
		// vector 1 can
		// be calculated
		final double dRotateX = (ptToVector.x * dCosAlpha)
				- (ptToVector.y * dSinAlpha);
		final double dRotateY = (ptToVector.x * dSinAlpha)
				+ (ptToVector.y * dCosAlpha);

		// 4. Now calculate the Theta trig values
		final TrigValues val = new TrigValues();
	 ...
	 *         given point
	 */
	public final LineSeg getParallelLineSegThroughPoint(final Point ptLoc) {
		if (isHorizontal()) {
			...
			final Point ptProj = perpIntersect(ptLoc.x, ptLoc.y);
			final long nHeight = Math.round(ptProj.getDistance(ptLoc));
			final Sign position = positionRelativeTo(ptLoc);
	 ...
	 */
	public static double[] getLineEquation(final double x1, final double y1,
			final double x2, final double y2) {
		final double equation[] = new double[3];
		for (int i = 0; i < 3; i++) {
			equation[i] = 0;
		}
	...
	public double[] getEquation() {
		final PrecisionPoint preciseOrigin = new PrecisionPoint(origin);
		final PrecisionPoint preciseTerminus = new PrecisionPoint(terminus);
	 ...
	 */
	public PointList getLinesIntersections(final LineSeg line) {
		final PrecisionPointList intersections = new PrecisionPointList();
		double temp[] = getEquation();
		final double a1 = temp[0];
		final double b1 = temp[1];
		final double c1 = temp[2];

		temp = line.getEquation();
		final double a2 = temp[0];
		final double b2 = temp[1];
		final double c2 = temp[2];
		// Cramer's rule for the system of linear equations
		final double det = a1 * b2 - b1 * a2;
		if (det == 0) {
	 ...
	 */
	public PointList getLineIntersectionsWithEllipse(
			final Rectangle ellipseBounds) {
		final PointList intersections = new PrecisionPointList();
		final PrecisionPoint preciseOrigin = new PrecisionPoint(origin);
		final PrecisionPoint preciseTerminus = new PrecisionPoint(terminus);
		final PrecisionRectangle preciseEllipseBounds = new PrecisionRectangle(
				ellipseBounds);
		if (preciseEllipseBounds.preciseWidth == 0
				|| preciseEllipseBounds.preciseHeight == 0) {
			return intersections;
		}
		final PrecisionPoint ellipsePreciseCenter = new PrecisionPoint(
				preciseEllipseBounds.getCenter());
		final double xl1 = preciseOrigin.preciseX
				- ellipsePreciseCenter.preciseX;
		final double xl2 = preciseTerminus.preciseX
				- ellipsePreciseCenter.preciseX;
		final double yl1 = preciseOrigin.preciseY
				- ellipsePreciseCenter.preciseY;
		final double yl2 = preciseTerminus.preciseY
				- ellipsePreciseCenter.preciseY;
		final double[] equation = LineSeg.getLineEquation(xl1, yl1, xl2, yl2);

		if (equation.length < 3 || (equation[0] == 0 && equation[1] == 0)) {
			return intersections;
		}

		final double a = equation[0];
		final double b = equation[1];
		final double c = equation[2];
		final double w = preciseEllipseBounds.preciseWidth;
		final double h = preciseEllipseBounds.preciseHeight;
		...
		if (b == 0) {
			...
			// or
			// x^2*xA+x*xB+xC=0
			final double xA = Math.pow(h, 2) + Math.pow((w * a) / b, 2);
			final double xB = (-2) * Math.pow(w, 2) * a * c / Math.pow(b, 2);
			final double xC = Math.pow(w * c / b, 2) - Math.pow(h * w / 2, 2);
			final double xD = Math.pow(xB, 2) - 4 * xA * xC;

			if (xD < 0) {
				return intersections;
			}

			final double x1 = (-xB + Math.sqrt(xD)) / (2 * xA);
			final double x2 = (-xB - Math.sqrt(xD)) / (2 * xA);
	...
	public PointList getLineIntersectionsWithLineSegs(final PointList points) {
		...
		if (points.size() < 2) {
			...
			for (int i = 0; i < points.size() - 1; i++) {
				final LineSeg seg = new LineSeg(points.getPoint(i),
						points.getPoint(i + 1));
				final PointList currentIntersections = getLinesIntersections(seg);
				for (int j = 0; j < currentIntersections.size(); j++) {
					final Point intersection = currentIntersections.getPoint(j);
