...
import org.soyatec.tooling.gef.anchor.OrthogonalConnectionAnchor;
import org.soyatec.tooling.gef.utils.FigureUtilities;
import org.soyatec.tooling.gef.utils.OrthogonalRouterUtilities;
import org.soyatec.tooling.gef.utils.PointListUtilities;

/**
 * @author sshaw/aboyko
 * 
 *         RectilinearRouter which routes the Connection so that the lines are
...
public class RectilinearRouter extends ObliqueRouter {
	...
	protected boolean checkSelfRelConnection(final Connection conn,
		...
		final boolean selfConn = super.checkSelfRelConnection(conn, newLine);
		if (selfConn) {
	 ...
	 */
	public void routeLine(final Connection conn, int nestedRoutingDepth,
			final PointList newLine) {
		final boolean skipNormalization = (routerFlags & ROUTER_FLAG_SKIPNORMALIZATION) != 0;
		 ...
		 */
		final Point lastStartAnchor = newLine.removePoint(0);
		final Point lastEndAnchor = newLine.removePoint(newLine.size() - 1);
		...
		final Dimension tolerance = new Dimension(3, 0);
		 ...
		 */
		if (newLine.size() == 2) {
			final Ray middleSeg = new Ray(newLine.getFirstPoint(),
					newLine.getLastPoint());
		...
		if (nestedRoutingDepth < maxNestedRoutingDepth
				&& !isValidRectilinearLine(conn, newLine)) {
			routeLine(conn, ++nestedRoutingDepth, newLine);
		}
	}

	/**
	 * Rectilinear polyline is invalid if: 1. First bend point is within the
	 * source 2. Last bend point is within the target 3. First bend point and
	 * source anchor are on different sides of the source shape 4. Last bend
	 * point and target anchor are on different sides of the target shape
	 * 
	 * @param conn
	 *            connection
	 * @param line
	 ...
	 */
	private boolean isValidRectilinearLine(final Connection conn,
			final PointList line) {
		if (!(conn.getSourceAnchor().getOwner() instanceof Connection)) {
			final Rectangle source = new PrecisionRectangle(
			...
			final int firstSegmentOrientation = line.getFirstPoint().x == line
		...
		if (!(conn.getTargetAnchor().getOwner() instanceof Connection)) {
			final Rectangle target = new PrecisionRectangle(
					FigureUtilities.getAnchorableFigureBounds(conn
			...
			final int lastSegmentOrientation = line.getLastPoint().x == line
					.getPoint(line.size() - 2).x ? PositionConstants.VERTICAL
					: PositionConstants.HORIZONTAL;
			if (getOutisePointOffRectanglePosition(
					line.getPoint(line.size() - 2), target) != getAnchorLocationBasedOnSegmentOrientation(
	 ...
	 */
	private void removePointsInViews(final Connection conn,
			final PointList newLine, final Point start, final Point end) {
		 ...
		 */
		final PrecisionRectangle source = conn.getSourceAnchor().getOwner() != null ? new PrecisionRectangle(
				FigureUtilities.getAnchorableFigureBounds(conn
						.getSourceAnchor().getOwner())) : null;
		final PrecisionRectangle target = conn.getTargetAnchor().getOwner() != null ? new PrecisionRectangle(
				FigureUtilities.getAnchorableFigureBounds(conn
		 ...
		 */
		if (newLine.size() == 0) {
			final Dimension tolerance = new Dimension(1, 0);
			final int toleranceValue = tolerance.width;
			...
			if (Math.abs(lastRemovedFromSource.x - lastRemovedFromTarget.x) < toleranceValue) {
			...
			} else if ((conn.getSourceAnchor() instanceof BaseConnectionAnchor
					&& "".equals(((BaseConnectionAnchor) conn.getSourceAnchor())
							.getTerminal()) && (conn.getTargetAnchor() instanceof BaseConnectionAnchor && ""
					.equals(((BaseConnectionAnchor) conn.getTargetAnchor())
							.getTerminal())))) {
				/*
				...
				if (lastRemovedFromSource != null
					...
					final double startX = Math.max(source.preciseX(),
							target.preciseX());
					final double endX = Math.min(
							source.preciseX() + source.preciseWidth(),
							target.preciseX() + target.preciseWidth());
					final double startY = Math.max(source.preciseY(),
							target.preciseY());
					final double endY = Math.min(
							source.preciseY() + source.preciseHeight(),
	 ...
	 * @return
	 */
	private int getAnchorOffRectangleDirection(final Point anchorPoint,
	 ...
	 * @return
	 */
	private Dimension getTranslationValue(final int position,
			final int xFactorValue, final int yFactorValue) {
		final Dimension translationDimension = new Dimension();
	 ...
	 * @return
	 */
	private int getOffShapeDirection(final int anchorRelativeLocation) {
	...
	protected void resetEndPointsToEdge(final Connection conn,
		...
		final PrecisionRectangle source = sourceBoundsRelativeToConnection(conn);
		final PrecisionRectangle target = targetBoundsRelativeToConnection(conn);
		...
		if (line.size() == 0) {
			 ...
			 */
			// Need to add 2 dumb points to ensure that
			// RouterHelper#resetEndPointsToEdge works
			 ...
			 */
			final Point offStart = line.getFirstPoint();
			final Point offEnd = line.getLastPoint();
			final Dimension offsetDim = offStart.getDifference(offEnd).scale(
					0.5);
			...
			final Point start = line.getFirstPoint();
			final Point end = line.getLastPoint();
			if (conn.getSourceAnchor() instanceof OrthogonalConnectionAnchor) {
				 ...
				 * rectilinear.
				 */
				final PrecisionPoint reference = new PrecisionPoint(start);
				conn.getSourceAnchor().getOwner()
						.translateToAbsolute(reference);
				final PrecisionPoint anchorLocation = new PrecisionPoint(conn
			...
			if (conn.getTargetAnchor() instanceof OrthogonalConnectionAnchor) {
				line.addPoint(OrthogonalRouterUtilities
						.getOrthogonalLineSegToAnchorLoc(conn,
								conn.getTargetAnchor(), end).getOrigin());
			} else {
				/*
				 * If anchor is not supporting orthogonal connections we'll use
				 * the oblique connection anchors and then convert it to
				 * rectilinear.
				 */
				final PrecisionPoint reference = new PrecisionPoint(end);
				conn.getSourceAnchor().getOwner()
						.translateToAbsolute(reference);
				final PrecisionPoint anchorLocation = new PrecisionPoint(conn
	 ...
	 * segments from nearly straight segments.
	 * 
	 * @param line
	 *            polyline
	 * @param tolerance
	 ...
	 */
	private void normalizeToStraightLineTolerance(final PointList line,
			final int tolerance) {
		for (int i = 0; i < line.size() - 1; i++) {
			final Point pt1 = line.getPoint(i);
			final Point pt2 = line.getPoint(i + 1);
	 ...
	 */
	private PrecisionRectangle sourceBoundsRelativeToConnection(
			final Connection conn) {
		final PrecisionRectangle source = new PrecisionRectangle(conn
	 ...
	 *         connection's coordinate system
	 */
	private PrecisionRectangle targetBoundsRelativeToConnection(
			final Connection conn) {
		final PrecisionRectangle target = new PrecisionRectangle(conn
	 ...
	 */
	private int getOutisePointOffRectanglePosition(final Point p,
			final Rectangle r) {
	...
	/**
	 * Given the coordinates of the connection anchor point the shape's
	 * rectangle and the orientation of the first rectilinear connection segment
	 * that comes out from the anchor point the method detemines on which
	 * geographic side of the rectangle the anchor point is located on.
	 * 
	 * @param anchorPoint
	 ...
	 */
	private int getAnchorLocationBasedOnSegmentOrientation(
			final Point anchorPoint, final Rectangle rectangle,
			final int segmentOrientation) {
	 ...
	 *            polyline's points
	 */
	private boolean removeRedundantPoints(final PointList line) {
		final int initialNumberOfPoints = line.size();
		if (line.size() > 2) {
			final PointList newLine = new PointList(line.size());
			newLine.addPoint(line.removePoint(0));
			while (line.size() >= 2) {
				final Point p0 = newLine.getLastPoint();
				final Point p1 = line.getPoint(0);
				final Point p2 = line.getPoint(1);
