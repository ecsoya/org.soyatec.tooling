...
import org.soyatec.tooling.gef.figures.IOvalAnchorableFigure;
import org.soyatec.tooling.gef.figures.IPolygonAnchorableFigure;
import org.soyatec.tooling.gef.figures.IPolylineConnection;
import org.soyatec.tooling.gef.utils.LineSeg;
...
public class ObliqueRouter extends BendpointConnectionRouter {
	...
	static public class ArrayListMap {
		...
		public ArrayList<Object> get(final Object key) {
			...
			final ArrayList<Object> v = new ArrayList<Object>(1);
			v.add(value);
			return v;
		}
		...
		public void put(final Object key, final Object value) {
			final Object arrayListObject = map.get(key);
			if (arrayListObject == null) {
				map.put(key, value);
				return;
			}
			if (arrayListObject instanceof ArrayList) {
				final ArrayList<Object> arrayList = (ArrayList<Object>) arrayListObject;
		...
		public void remove(final Object key, final Object value) {
			...
			if (arrayListObject != null) {
				if (arrayListObject instanceof ArrayList) {
					...
					final ArrayList arrayList = (ArrayList) arrayListObject;
	...
	private final ArrayListMap selfRelConnections = new ArrayListMap();
	private final ArrayListMap intersectingShapesConnections = new ArrayListMap();

	private class ArrayListKey {

		private final ConnectionAnchor connectAnchor1;
		private final ConnectionAnchor connectAnchor2;

		ArrayListKey(final Connection conn) {
		...
		public boolean equals(final Object object) {
			...
			if (object instanceof ArrayListKey) {
				...
				final ConnectionAnchor lk1 = listKey.getSourceAnchor();
				final ConnectionAnchor lk2 = listKey.getTargetAnchor();

				isEqual = (lk1.equals(connectAnchor1) && lk2
						.equals(connectAnchor2))
						|| (lk1.equals(connectAnchor2) && lk2
								.equals(connectAnchor1));
			}
			return isEqual;
		}
	}
	 ...
	 * the
	 */
	public boolean isClosestDistance(final Connection conn) {
		if (conn instanceof IPolylineConnection) {
			return ((IPolylineConnection) conn).isClosestDistanceRouting();
		}
	 ...
	 */
	protected boolean isReorienting(final Connection conn) {
	 ...
	 */
	final public void route(final Connection conn) {
		if (conn.isVisible()) {
	 ...
	 */
	public void routeBendpoints(final Connection conn) {
		if ((conn.getSourceAnchor() == null)
				|| (conn.getTargetAnchor() == null)) {
			return;
		}

		final PointList points = calculateBendPoints(conn);

		// points could be null if routing is already finished in
		// calculateBendPoints
		if (points != null) {
	 ...
	 * Clients can override this method to introduce calculated bend points on
	 * the connection
	 * 
	 * @param conn
	 *            the connection to get the bend points for
	 * @return bend points as a Point List
	 */
	protected PointList calculateBendPoints(final Connection conn) {
		final RouterHelper helper = RouterHelper.getInstance();
	 ...
	 * 
	 * <li>If the connection is completely within one container (doesn't matter
	 * if the container is nested), then GEF's router is used, meaning that the
	 * connection will be appropriately re-routed in case when an obstacle is
	 * being placed on the way.
	 * 
	 * <li>If the connection spans between two containers, then this method
	 * returns null and the old GMF algorithm for avoiding obstructions is used
	 * (meaning that re-routing will not happen right away when an object is
	 ...
	 * 
	 * <p>
	 * Self connections and connections whose source and target intersect are
	 * dealt with in <code>routeLine</code> and avoid obstructions is irrelevant
	 * (just like when non-GEF avoid obstructions router is used).
	 * 
	 * <p>
	 * Known issue: if an obstacle contains a connection start or end point,
	 * that obstacle is ignored, meaning that the connection can be routed
	 * through it.
	 * 
	 * @param conn
	 * @return true if routing was done by GEF algorithm, false otherwise
	 */
	public boolean routeAroundObstructions_GEF(final Connection conn) {
		final RouterHelper helper = RouterHelper.getInstance();
		boolean routed = false;
		if (helper.getUseGEFRouter()) {
			final ShortestPathConnectionRouter spcr = helper.getConnRouter(
					conn, true);
			if (spcr != null) {
				// Do routing only if spcr says there is something to route, or
				// if
				// this is the first time conn is routed by spcr
				if (spcr.isDirty() || !spcr.containsConnection(conn)) {
					// add conn to spcr if needed
					// (in our case, spcr takes into account only end points,
					// manual bendpoints
					// in constraint will be ignored)
					if (!spcr.containsConnection(conn)) {
						helper.setConstraint(spcr, conn, null);
					}
					// spcr has to ignore invalidation of connections that are
					// rooted as a
					// result of this call (otherwise, invalidation of those
					// connections would dirty spcr
					// therefore causing routing to be done again)
					spcr.setIgnoreInvalidate(true);

					@SuppressWarnings("unchecked")
					final List<Path> allPaths = spcr.getPathsAfterRouting();
					if (allPaths != null && allPaths.size() > 0) {
						routed = true;
						IFigure container = helper.getSourceContainer(conn);
						// Source and target containers are the same (we know
						// since GEF router is used)
						// Exception: user is moving a connection anchor and it
						// is currently not inside any figure.
						if (container == null) {
							container = helper.getTargetContainer(conn);
						}
						if (container != null) { // should never be null at this
													// point
							PointList points;
							for (int i = 0; i < allPaths.size(); i++) {
								final Path path = allPaths.get(i);
								final Connection currentConn = (Connection) path.data;
								points = new PointList();
								// spcr needed path coordinates to be relative
								// to the container.
								// Now translate them back to be relative to the
								// connection.
								for (int j = 0; j < path.getPoints().size(); j++) {
									final PrecisionPoint pt = new PrecisionPoint(
											path.getPoints().getPoint(j));
									container.translateToAbsolute(pt);
									currentConn.translateToRelative(pt);
									points.addPoint(pt);
								}
								// Adjust start and end points, check for self
								// connection, and source and target
								// intersecting
								// Problem: framework is designed for
								// connections to be routed one at the time, but
								// GEF's
								// algorithm routes several connections at once.
								// Now we have to check if currentConn is routed
								// by the same router as conn, it could be
								// routed by child (e.g. rectilinear router), or
								// conn could be
								// routed by child and currentConn by this
								// router.
								final ConnectionRouter currentConnRouter = currentConn
										.getConnectionRouter();
								if (!currentConnRouter.equals(conn
										.getConnectionRouter())) {
									if (currentConnRouter instanceof ObliqueRouter) {
										((ObliqueRouter) currentConnRouter)
												.routeLine(currentConn, 0,
														points);
									} else if ((currentConnRouter instanceof FanRouter)
											&& (((FanRouter) currentConnRouter)
													.getRouter() instanceof ObliqueRouter)) {
										// this handles the case when
										// ObliqueRouter is delegate of
										// FanRouter
										((ObliqueRouter) ((FanRouter) currentConnRouter)
												.getRouter()).routeLine(
												currentConn, 0, points);
									} else {
										routeLine(currentConn, 0, points);
									}

									// (another way would be to revalidate
									// currentConn so it would be routed by its
									// own router
									// through the process of validation, but
									// would have to prevent endless loop)
								} else {
									routeLine(currentConn, 0, points);
								}

								// Check if this path really changed or not.
								// This check will reduce the number of
								// revalidation calls, but it is not necessary
								// (even if revalidation is called on a
								// connection that didn't change,
								// spcr.getIsDirty() will return false and
								// routing will not happen)
								final PointList oldPoints = currentConn
										.getPoints();
								boolean route = false;
								if (oldPoints == null
									...
									for (int j = 0; j <= oldPoints.size() - 1; j++) {
										if (oldPoints.getPoint(j).x != points
												.getPoint(j).x
												|| oldPoints.getPoint(j).y != points
														.getPoint(j).y) {
											route = true;
											break;
										}
								...
								if (route) {
									...
									// don't revalidate conn since it already
									// went through the whole process of layout
									if (conn != currentConn) {
										// Revalidate to ensure that currentConn
										// goes through the complete layout.
					...
					// Still, call setPoints since it ensures calculating bounds
	 ...
	 */
	protected boolean removePointsInViews(final Connection conn,
		...
		final IFigure sourceOwner = conn.getSourceAnchor().getOwner();
		final IFigure targetOwner = conn.getTargetAnchor().getOwner();
		if (sourceOwner == null) {
			return false;
		}
		if (targetOwner == null) {
		...
		final PointList newPoints = new PointList(newLine.size());
		for (int i = 0; i < newLine.size(); i++) {

			final Point pt = newLine.getPoint(i);
			if (i == 0 || i == newLine.size() - 1) {
				newPoints.addPoint(pt);
			} else if ((startPolygon == null || !PointListUtilities
					.containsPoint(startPolygon, pt))
					&& (endPolygon == null || !PointListUtilities
							.containsPoint(endPolygon, pt))) {
				newPoints.addPoint(pt);
			} else {
				bChanged = true;
			}
	...
	protected PointList getFigurePolygon(final IFigure owner,
		...
		if (owner instanceof IOvalAnchorableFigure) {
			final Rectangle rect = new PrecisionRectangle(
					((IOvalAnchorableFigure) owner).getOvalBounds());
			...
			final PointList points = ((IPolygonAnchorableFigure) owner)
					.getPolygonPoints();
			for (int index = 0; index < points.size(); index++) {
				final Point point = points.getPoint(index).getCopy();
				owner.translateToAbsolute(point);
				conn.translateToRelative(point);
				polygon.addPoint(point);
			}
		} else {
			final Rectangle rect = new PrecisionRectangle(owner.getBounds());
			owner.translateToAbsolute(rect);
			conn.translateToRelative(rect);
			polygon.addPoint(rect.getTopLeft());
			polygon.addPoint(rect.getTopRight());
	...
	/**
	 * Helper method for "route" to just do the core routing of this router
	 * without any additional ideology (i.e. no closest distance, obstructions
	 * routing).
	 */
	public void routeLine(final Connection conn, final int nestedRoutingDepth,
			final PointList newLine) {
	...
	/**
	 * Checks if source shape and target shape of the connection intersect (only
	 * intersect - not one contained in another) and if they are calculates the
	 * bendpoints for the connection. Calculated bendpoints are stored in
	 * <code>newLine</code>. Initially <code>newLine</code> contains the list of
	 * bendpoints calculated by the router, however if for intersecting shapes
	 * we have a default connection (i.e. no extra bendpoints), bendpoints will
	 * be calculated and <code>newLine</code> will be cleared and calculated
	 * bendpoints will be stored there.
	 * 
	 * Criterias for calculation of bendpoints for connection between 2
	 * intersecting shapes: 1. No extra bendpoints introduced by user (only
	 * source and target anchor points present) 2. Source and target shapes
	 * intersect (one contained in another = do not intersect)
	 * 
	 * @param conn
	 *            connection
	 * @param newLine
	 *            list to store calculated bendpoints (contains bendpoints read
	 *            from the model initially
	 * @return <code>true</code> if bendpoints were calculated here for
	 *         intersecting shapes
	 */
	protected boolean checkShapesIntersect(final Connection conn,
			final PointList newLine) {
		if (conn.getSourceAnchor().getOwner() == null
		...
		if (newLine.size() < 3) {
			final PrecisionRectangle sourceBounds = getShapeBounds(conn
					.getSourceAnchor().getOwner());
			final PrecisionRectangle targetBounds = getShapeBounds(conn
	...
	private PrecisionRectangle getShapeBounds(final IFigure figure) {
		if (figure instanceof IOvalAnchorableFigure) {
			return new PrecisionRectangle(
					((IOvalAnchorableFigure) figure).getOvalBounds());
	...
	private int getSourcePositionFromConnectionRectangle(
			final Rectangle connRectangle, final Rectangle sourceRect,
			final int position) {
		Dimension diff = null;
		switch (position) {
	 ...
	 */
	private void getConnectionPoints(final Rectangle connRect,
			final int position, final int sourcePosition, final PointList line) {
	 ...
	 *            dimension
	 */
	private void absDimension(final Dimension d) {
	 ...
	 */
	private void getVerticesForIntersectingShapes(final Connection conn,
			final PointList newLine) {
		final Object key = getIntersectingShapesConnectionKey(conn);
		int nSelfIncr = 0;
		int nIndex = 0;
		/*
		 ...
		 */
		final PrecisionPoint selfrelsizeincr = new PrecisionPoint(
				SELFRELSIZEINCR, 0);
		...
		final PrecisionRectangle sourceRect = getShapeBounds(sourceFig);
		...
		final PrecisionRectangle targetRect = getShapeBounds(targetFig);
		targetFig.translateToAbsolute(targetRect);
		conn.translateToRelative(targetRect);
		@SuppressWarnings("deprecation")
		final PrecisionRectangle union = sourceRect.getPreciseCopy().union(
				targetRect);
		...
		final Rectangle intersection = sourceRect.getCopy().intersect(
				targetRect);
		...
		int minArea = 0;
		final Point unionTopLeft = union.getTopLeft();
		final Point unionTopRight = union.getTopRight();
		final Point unionBottomRight = union.getBottomRight();
		final Point unionBottomLeft = union.getBottomLeft();

		if (!unionTopLeft.equals(sourceRect.getTopLeft())
				&& !unionTopLeft.equals(targetRect.getTopLeft())) {
			final Dimension diffVector = unionTopLeft
					.getDifference(intersection.getTopLeft());
			absDimension(diffVector);
			final int areaTopLeft = diffVector.getArea();
		...
		if (!unionTopRight.equals(sourceRect.getTopRight())
			...
			final Dimension diffVector = unionTopRight
					.getDifference(intersection.getTopRight());
			absDimension(diffVector);
			final int areaTopRight = diffVector.getArea();
		...
		if (!unionBottomRight.equals(sourceRect.getBottomRight())
			...
			final Dimension diffVector = unionBottomRight
					.getDifference(intersection.getBottomRight());
			absDimension(diffVector);
			final int areaBottomRight = diffVector.getArea();
		...
		if (!unionBottomLeft.equals(sourceRect.getBottomLeft())
			...
			final Dimension diffVector = unionBottomLeft
					.getDifference(intersection.getBottomLeft());
			absDimension(diffVector);
			final int areaBottomLeft = diffVector.getArea();
		 ...
		 */
		final int sourcePosition = getSourcePositionFromConnectionRectangle(
				connArea, sourceRect, position);

		if (position != PositionConstants.NONE) {
			/*
			 * Determine the value by which the connection area has to become
			 * primary precise connection area. The value is chosen to be such
			 * that connections made from shapes intersecting on the same edge
			 * don't overlap
			 */
			final PrecisionPoint translateExpansion = new PrecisionPoint(
					Math.max(connArea.width, connArea.height), 0);
			translateExpansion.setPreciseX(Math.pow(
					translateExpansion.preciseX(), 0.8));
			translateExpansion.updateInts();

			/*
			 * Transform rough connection area to primary precise connection
			 * area
			 */
		...
		final PrecisionPoint ptS2 = new PrecisionPoint(newLine.getPoint(0));
		PrecisionPoint ptS1 = new PrecisionPoint(conn.getSourceAnchor()
				.getReferencePoint());
		conn.translateToRelative(ptS1);
		final Point ptAbsS2 = new Point(ptS2);
		conn.translateToAbsolute(ptAbsS2);
		PrecisionPoint ptEdge = new PrecisionPoint(conn.getSourceAnchor()
				.getLocation(ptAbsS2));
		conn.translateToRelative(ptEdge);
		ptS1 = new PrecisionPoint(getStraightEdgePoint(ptEdge, ptS1, ptS2));

		final PrecisionPoint ptE2 = new PrecisionPoint(newLine.getPoint(newLine
				.size() - 1));
		PrecisionPoint ptE1 = new PrecisionPoint(conn.getTargetAnchor()
				.getReferencePoint());
		conn.translateToRelative(ptE1);
		final PrecisionPoint ptAbsE2 = (PrecisionPoint) ptE2.getCopy();
	 ...
	 *            union of intersecting shapes
	 */
	private void getPrimaryPreciseConnectionArea(final Rectangle r,
			final int size, final int positionOfConnArea) {
		r.expand(size, size);
		if (r.width < r.height) {
			r.height -= size;
	...
	/**
	 * getStraightEdgePoint Gets the anchored edge point that intersects with
	 * the given line segment.
	 * 
	 * @param ptEdge
	 *            Point on the edge of the end shape in relative coordinates
	 * @param ptRef1
	 *            Point that is the first reference in relative coordinates
	 * @param ptRef2
	 *            Point that is the second reference in relative coordiantes
	...
	protected static Point getStraightEdgePoint(final Point ptEdge,
		...
		final LineSeg lineSeg = new LineSeg(ptRef1, ptRef2);

		final Point ptProj = lineSeg.perpIntersect(ptEdge.x, ptEdge.y);
	...
	/**
	 * Reset the end points of the connection line to the appropriate anchor
	 * position on the start and end figures.
	 */
	protected void resetEndPointsToEdge(final Connection conn,
			final PointList newLine) {
	 ...
	 */
	protected boolean checkSelfRelConnection(final Connection conn,
			final PointList newLine) {
		if ((conn.getSourceAnchor().getOwner() == conn.getTargetAnchor()
				.getOwner()) && newLine.size() < 4) {
	...
	/**
	 * Method removeSelfRelConnection. Removes the given connection from the
	 * self relation hash map
	 * 
	 * @param conn
	 *            Connection to remove from the map
	 */
	private void removeSelfRelConnection(final Connection conn) {
		...
		final ArrayListKey connectionKey = new ArrayListKey(conn);
		final ArrayList<Object> connectionList = selfRelConnections
				.get(connectionKey);
		if (connectionList != null) {
	...
	/**
	 * Method removeIntersectingShapesConnection. Removes the given connection
	 * from the intersecting shapes connections hash map
	 * 
	 * @param conn
	 *            Connection to remove from the map
	 */
	private void removeIntersectingShapesConnection(final Connection conn) {
		if (conn.getSourceAnchor() == null || conn.getTargetAnchor() == null
				|| conn.getSourceAnchor().getOwner() == null
				|| conn.getTargetAnchor().getOwner() == null) {
			return;
		...
		final Object key = getIntersectingShapesConnectionKey(conn);
		final ArrayList<Object> connectionList = intersectingShapesConnections
				.get(key);
		if (connectionList != null) {
			final int index = connectionList.indexOf(conn);
			if (index == -1) {
	 ...
	 * @return hash code
	 */
	private Object getIntersectingShapesConnectionKey(final Connection conn) {
		return new Integer(conn.getSourceAnchor().getOwner().hashCode()
				^ conn.getTargetAnchor().getOwner().hashCode());
	}

	/**
	 * Method insertSelfRelVertices. This method will create a routed line that
	 * routes from and to the same figure.
	 * 
	 * @param conn
	 * @param newLine
	 */
	protected void getSelfRelVertices(final Connection conn,
			final PointList newLine) {
		if (conn.getSourceAnchor().getOwner() == null) {
			return;
		}
		...
		int nSelfIncr = 0;
		int nIndex = 0;
		ArrayList<Object> connectionList = selfRelConnections
				.get(connectionKey);
		if (connectionList != null) {
			if (!connectionList.contains(conn)) {
		...
		final Dimension selfrelsizeincr = new Dimension(SELFRELSIZEINCR, 0);

		final IFigure owner = conn.getSourceAnchor().getOwner();
		final Rectangle bBox = owner.getClientArea();
		...
		switch (nIndex % 8) {
		case 0:
			getCornerSelfRelVertices(conn, bBox, newLine, nSelfIncr, 1, 1,
					bBox.getBottomRight());
			break;
		case 1:
			getVerticalSelfRelVertices(conn, bBox, newLine, nSelfIncr, 1,
					bBox.getBottom());
			break;
		case 2:
			getCornerSelfRelVertices(conn, bBox, newLine, nSelfIncr, -1, 1,
			...
			break;
		case 3:
			getHorizontalSelfRelVertices(conn, bBox, newLine, nSelfIncr, -1,
					bBox.getLeft());
			break;
		case 4:
			getCornerSelfRelVertices(conn, bBox, newLine, nSelfIncr, -1, -1,
		...
		final Point ptS2 = newLine.getPoint(0);
		...
		final Point ptE2 = newLine.getPoint(newLine.size() - 1);
		Point ptE1 = conn.getTargetAnchor().getReferencePoint();
		conn.translateToRelative(ptE1);
		final Point ptAbsE2 = new Point(ptE2);
		conn.translateToAbsolute(ptAbsE2);
	...
	/**
	 * Method getCornerSelfRelVertices. Retrieves the relation points for the
	 * self relation given a corner point and direction factors.
	 * 
	 * @param bBox
	 *            Rectangle representing the shape extents to create the self
	 *            relation around.
	 * @param newLine
	 *            PointList of the line to receive the new points
	 * @param nOffset
	 *            Incremental offset of the self relation to prevent overlapping
	 *            relations.
	 * @param nXDir
	 *            int Direction (either -1, 1) indicating the horizontal growth
	 *            direction
	 * @param nYDir
	 *            int Direction (either -1, 1) indicating the vertical growth
	 *            direction
	 * @param ptOrient
	 *            Point which represents the starting location for the self
	 *            relation to grow from.
	 */
	private void getCornerSelfRelVertices(final Connection conn,
			final Rectangle bBox, final PointList newLine, final int nOffset,
			final int nXDir, final int nYDir, final Point ptOrient) {

		final int x = ptOrient.x;
		final int y = bBox.getCenter().y + (nYDir * bBox.height / 4);
		final Point p1 = new Point(x, y);
		newLine.addPoint(p1);

		int xNew, yNew;

		final Dimension selfrelsizeinit = new Dimension(SELFRELSIZEINIT, 0);

		xNew = x + (nXDir * (selfrelsizeinit.width + nOffset));
		final Point p2 = new Point(xNew, y);
		newLine.addPoint(p2);

		yNew = ptOrient.y + (nYDir * (selfrelsizeinit.width + nOffset));
		final Point p3 = new Point(xNew, yNew);
		newLine.addPoint(p3);

		xNew = ptOrient.x - (nXDir * bBox.width / 4);
		final Point p4 = new Point(xNew, yNew);
		newLine.addPoint(p4);

		yNew = ptOrient.y;
		final Point p5 = new Point(xNew, yNew);
		newLine.addPoint(p5);
	}
	 ...
	 *            relation to grow from.
	 */
	private void getVerticalSelfRelVertices(final Connection conn,
			final Rectangle bBox, final PointList newLine, final int nOffset,
			final int nDir, final Point ptOrient) {

		final int nWidth = bBox.width / 4;

		final int x = ptOrient.x - nWidth / 2;
		final int y = ptOrient.y;
		final Point p1 = new Point(x, y);
		newLine.addPoint(p1);
		...
		final Point p2 = new Point(x, yNew);
		newLine.addPoint(p2);
	 ...
	 *            relation to grow from.
	 */
	private void getHorizontalSelfRelVertices(final Connection conn,
			final Rectangle bBox, final PointList newLine, final int nOffset,
			final int nDir, final Point ptOrient) {

		final int nHeight = bBox.height / 4;

		final int y = ptOrient.y - nHeight / 2;
		final int x = ptOrient.x;
		final Point p1 = new Point(x, y);
		newLine.addPoint(p1);
